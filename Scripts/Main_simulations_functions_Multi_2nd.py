...
"""
'''
Script containing the main functions calling the other scripts to calculate the LCAs in Mono-Dimensional Sampling, 
...
def calculateLCA_1param_parallel(constant_inputs,
                                 param_set) :   
        ...
        for param in Biodict:  
            ...
            for index in range(0, len(names_param_bio)):
        ...
        for param in Tech_opdict:  
            ...
            for index in range(new_start,new_start+len(names_param_op)):
        ...
        new_start = new_start+len(names_param_op)
        ...
        #optimization_performance = LCI[3]  # kg dw.d-1   #No need
...
def sampling_func_montecarlo(Tech_opdict_distributions,
                  Biodict_distributions,
                  Locationdict_distributions, 
                  Physicdict_distributions,
                  size):
  
    """Function which creates a Monte Carlo Sample of uncertain parameters.
    In the current model, It is used to generate samples of uncertain Techno-operational parameters after the distributions in the other dictionnaires are all set to "unique" """    
       # Creation of the problem
    ...
    for param in Tech_opdict_distributions_input:
        ...
        elif distrib == 'triang':  # Then bounds = Lower, mode, higher


            bounds.append([Tech_opdict_distributions_input[param][1][1], Tech_opdict_distributions_input[param][1][3],Tech_opdict_distributions_input[param][1][2]])
    ...
    for param in Biodict_distributions_input:
        ...
        if distrib == 'unif':  # then bounds = upper bound, lower bound
            ...
            bounds.append([Biodict_distributions_input[param][1][1], Biodict_distributions_input[param][1][3],Biodict_distributions_input[param][1][2]])
    ...
    for param in Locationdict_distributions_input:
        ...
        if distrib == 'unif':  # then bounds = upper bound, lower bound
            ...
            bounds.append([Locationdict_distributions_input[param][1][1], Locationdict_distributions_input[param][1][3],Locationdict_distributions_input[param][1][2]])
    ...
    for param in Physicdict_distributions_input:
        ...
        if distrib == 'unif':  # then bounds = upper bound, lower bound
            ...
            bounds.append([Physicdict_distributions_input[param][1][1], Physicdict_distributions_input[param][1][3],Physicdict_distributions_input[param][1][2]])

    names_param = names_param_bio + names_param_op + names_param_geo + names_param_phy
    ...
    return sample_array, names_param, names_param_op, names_param_bio, names_param_geo, names_param_phy
...
def final_function_simulations_sampling_2_SPECIAL(dict_mono_technosphere_lcas_nat,
                                                 ...
                                                 months_suitable_for_cultivation,  # Paper value
                                                 fraction_maxyield,  # Paper value
    ...
    """ Function which calculates, processes and returns the LCAs for 1 location defined with Locationdict, accroding to the Multi-dimensional sampling strategy."""
    ...
    # Sample once the parameters which are non specific to the Strain-compound pairs to know how many uncertain ones there are
    
    sample_montecarlo=sampling_func_montecarlo(Tech_opdict_distributions,
                      Biodict_distributions_unique,
                      Locationdict_distributions, 
                      Physicdict_distributions,
                      1)
    
    
    sample_mc = sample_montecarlo[0]

    names_param_mc = sample_montecarlo[1]

    names_param_mc_op = sample_montecarlo[2]

    names_param_mc_bio = sample_montecarlo[3]

    names_param_mc_geo = sample_montecarlo[4]

    names_param_mc_phy = sample_montecarlo[5]
    
    
    len_non_algae_param= len(names_param_mc)


    # For each strain-compound ppair, generate Montecalro combinations of unknown techo_operational parameters and put all the resulting combinations in 1 table 
    
    
    total_input =  np.full([0, len(names_param_algae)+len_non_algae_param], None)

    for algae_index in range(len(sample_algae)):
            
        
            clone = np.tile(sample_algae[algae_index], (size_montecarlo, 1))
                       
            empty_right_part = np.full([3, len_non_algae_param], None)
            
            
            sample_mc=sampling_func_montecarlo(Tech_opdict_distributions,
                     Biodict_distributions_unique,
                     Locationdict_distributions, 
                     Physicdict_distributions,
                     size_montecarlo)[0]
            
            ready_to_conc = np.concatenate((clone,sample_mc),axis=1)

            
            total_input=np.concatenate((total_input,ready_to_conc))

       
        
        
    # Little fix to match the variables'names of next function
         
    names_param_tot = names_param_algae+names_param_mc
    names_param_op =  names_param_mc_op
    names_param_bio = names_param_algae
    names_param_geo = names_param_mc_geo
    names_param_phy = names_param_mc_phy
    ...
    # Keep only the names of the methods
    ...
    # Note names_param replaced by names_param_tot
    ...
    # Calcluate all LCAS in parallel for this location and collect the results.
    arrayresult_raw =ray.get([calculateLCA_1param_parallel.remote(constant_inputs,
                                                                  param_set) for param_set in total_input])        
        
    print('ok2')
    # Names LCI
    
    
    names_LCI = arrayresult_raw[0][2] # for instance [0][2]
    
    
    # Build table of results with the results from ray.
        
    
    
    names_for_df = names_param_tot + names_LCI + names_suppl_info + names_methods_adjusted

        
    results_table_df = pd.DataFrame(np.array([row[0] for row in arrayresult_raw]), columns=names_for_df)

    # Rebuild list of tables of contributions
        
    intermediary_list_contrib = [row[1] for row in arrayresult_raw]
    
    del arrayresult_raw # Save memory
    
    
    # Indicate the Strain-compound associated to each individual


    results_table_df["alga"] = 0
    ...
    # Convert to float for statistical description
    
    results_table_df[[a for a in names_for_df if a not in toexclude ]] = results_table_df[[a for a in names_for_df if a not in toexclude ]].astype("float32", errors = 'raise')
    
    
    # List that will contain the dataframes with statiscial descriptions for all algae
    
    list_desc_stat_algae=[]
    

    # Statistical description for all strain-compounds
    
    for index_alga in range(len(sample_algae)):
        
        # Positions of the results for this strain compound in the table
        inf = index_alga*size_montecarlo
        sup = (index_alga+1)*size_montecarlo

        
        results_table_df.iloc[inf:sup, results_table_df.columns.get_loc("alga")] = index_alga
        ...
        desc_stat_results = results_table_df.iloc[inf:sup,[results_table_df.columns.get_loc(a) for a in names_for_df if a not in toexclude]].describe().astype("float32")
       
        desc_stat_results['alga'] = index_alga
        
        list_desc_stat_algae.append(desc_stat_results)
        
        

    # Build list of tables for contribution analysis
    ...
    del list_tables_contribution
    
    del list_tables_contribution_abs
    ...
    # List containing the statitical description of the contributions and for all algae : size = size of algae sample (1 algae =1 table with all methods) 
    
    list_total_desc_stat_contri_df =[]
    ...
    for index_alga in range(len(sample_algae)):
        ...
        # For all methods
        for index_meth in range(len(methods_selected)):
        
            #print('meth',index_meth)
            initial_dataframe = list_tables_contribution_abs_df[index_meth]
        
            # Lower and upper row index for this alga
            inf= index_alga*size_montecarlo
            sup= (index_alga+1)*size_montecarlo
    
    
            #initial_dataframe.iloc[inf:sup, initial_dataframe.columns.get_loc("alga")]=index_alga


            # This table only contains the observations for this 1 alga on this point

            contributions_for_alga_df = initial_dataframe.iloc[inf:sup]

            # statisitical description of the contributions for this alga
            
            desc_stat_contrib=contributions_for_alga_df.describe()
            ...
            separation_row[firstkey]=methods_selected[index_meth][-1]
            ...
            # Indicate the alga
            total_desc_stat_contri_df["alga"]=index_alga

            # Append to the list for all algae
        list_total_desc_stat_contri_df.append(total_desc_stat_contri_df)
